...
export async function generateInvoicePDF(invoice: Invoice): Promise<Buffer> {
  // Patch PDFKit to prevent Helvetica initialization
  try {
    const AnyPDF: any = PDFDocument as any;
    const proto = AnyPDF?.prototype;
    if (proto && !proto.__helveticaPatched) {
      const originalInitFonts = proto.initFonts;
      proto.initFonts = function () {
        ...
        // Do NOT call this.font('Helvetica') - skip default font
      };
      proto.__helveticaPatched = true;
      proto.__originalInitFonts = originalInitFonts;
    }
  } catch (err) {
    console.warn('[PDF] Font patch warning:', err);
  }
  ...
  // ✅ Register font BEFORE adding first page
  const fontCandidates = [
    path.join(process.cwd(), "public", "fonts", "FreeSans.otf"),
  ...
  for (const fontPath of fontCandidates) {
    if (fs.existsSync(fontPath)) {
      try {
        ...
        doc.font("Body");
        fontRegistered = true;
        console.log("[PDF] Using embedded font:", fontPath);
        break;
      } catch (err) {
        console.warn("[PDF] Failed to load font:", fontPath, err);
      }
    }
  }

  if (!fontRegistered) {
    throw new Error("No embeddable font found. Please ensure FreeSans.otf or DejaVuSans.ttf exists in public/fonts/");
  }
